...
const HealthStatusIndicator: React.FC<{ status: HealthStatus }> = ({
  status,
}) => {
...
const ServiceHealthCard: React.FC<{
  ...
  <div className={styles.serviceCard}>
    ...
    {service.responseTime && (
      <p className={styles.serviceResponse}>
...
const AlertLevelIndicator: React.FC<{ level: AlertLevel }> = ({ level }) => {
  ...
  return (
    <span className={`${styles.alertLevel} ${getLevelClass(level)}`}>
      {level}
    </span>
  );
};
    ...
    <div className={`${styles.dashboard} ${className || ""}`}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          {systemHealth && (
            <HealthStatusIndicator status={systemHealth.status} />
          )}
          <button
            onClick={() => void fetchDashboardData()}
        ...
        <div className={styles.overview}>
          <div className={styles.card}>
            ...
            <p className={styles.cardValue}>
              {Math.floor(systemHealth.uptime / 1000 / 60)}m
            </p>
          </div>
          <div className={styles.card}>
              ...
              >
                <div className={styles.alertHeader}>
                  <div className={styles.alertContent}>
                    <AlertLevelIndicator
                      level={alert?.level || AlertLevel.INFO}
                    />
        ...
        <div className={styles.section}>
          ...
          <div className={styles.analyticsGrid}>
            ...
            <div className={styles.card}>
              ...
              <p className={`${styles.cardValue} ${styles.cardValueSmall}`}>
                {analytics.topPages[0]?.path || "N/A"} (
                {analytics.topPages[0]?.views || 0} views)
              </p>
            </div>
          </div>
        </div>
